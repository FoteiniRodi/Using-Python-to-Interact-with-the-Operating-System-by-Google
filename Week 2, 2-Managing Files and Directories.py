...
How to examine if a file exists or not
        import os
...
How to examine if a file exists or not (another way)
        import os
        file= "file.dat"
...
How to view the contents of a directory (as a list)
        os.listdir()
        import os
...
Is it a file or a directory? (output: True or False)
...
How to Classify the contents of a directory as files or directories
        ...
        inside the initial directory, into one single name
...
abspath function, takes a file name and turns it into an absolute path
useful to access a file no matter what the current directory is
"""
import os
os.path.abspath("spider.txt")
...
find the current working directory
    ...
    is executing in, you can use the getcwd method. (for linux it is pwd, prints the current working directory)
...
import os
print(os.getcwd())
# This tells me the name of my current directory
...
for name in os.listdir("C:/Users/jimko"):   # FOR LOOP over the list. We iterate over the list contents (we examine each element one by one) 
    fullname = os.path.join(dir, name)      # we apply the join function of the os.path sub-module. It takes 2 arguments. The absolute path of the directory and the name of the file/file folder. The two names are joined in a way that the resulting path is recognized by many Operating Systems. We assign the Variable "fullname" to the result of this function (the joined names) 
...
def create_python_script(filename):                 # we create a new function that eventually will tell us the number of bytes for each Python file
...
print(create_python_script("program.py"))           # finally, we put our new function to work for a specific Python file
...
The new_directory function 
    ...
    -then creates a new empty file inside the new directory, and 
...
"""
import os
def new_directory(directory, filename):             # we create a new function that eventually will produce a list with the contents of the new directory       
    if not os.path.exists(directory):               # if the directory does not exist (which ha-ha is true because we want to create it. This is like "initializing")
        ...
        file = open(fullname, "w")                  # create a new file which will be named after the Variable "fullname". The new file is created inside the new directory
...
print(new_directory("PythonPrograms", "script.py")) #  # finally, we put our new function to work (and we provide the name for the directory and for the one file it contains)
